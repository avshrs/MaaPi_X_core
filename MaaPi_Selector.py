                    ...
                    for serv in self.runningServices:
                        if self.runningServices[serv]["ss_device_table_id"] == self.deviceList[dev]['dev_type_id']:
                            try:
                                ...
                                self.maapilogger.log("INFO",f"Devices sended to checkout readings {dev} | {self.deviceList[dev]['dev_user_name'].encode('utf-8').strip()} | {self.deviceList[dev]['dev_rom_id']} to {self.runningServices[serv]['ss_port']}")
                            except Exception as e:
